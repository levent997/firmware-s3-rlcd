...
- ✅ **3.3 中文字体** — CLOCK 视图 + 设置菜单已中文化。字体用 `u8g2_font_wqy14_t_gb2312`(文泉驿 14px,全 GB2312,含 ASCII,经 `drawUTF8` 渲染),flash 23.5%→29.5%(+~250KB,4MB 槽内无压力)
  - ✅ **CLOCK 视图**:星期(星期X)、日期(YYYY年M月D日)、元数据(第N周 / 全年第N天)、三张卡标签(温度/湿度/电量)、无时间提示全中文化。大字 HH:MM:SS 数字与秒 tick 保留
  - ✅ **设置菜单**:标题(设置)、各项标签(声音/重置统计/删除角色包/恢复出厂/重启/取消)、开关值(开/关)、确认页标题 + 正文全中文化。按键提示([KEY]/[BOOT] 等)保留英文(指物理键)
  - ✅ **收到的消息中文显示**:`protocol::sanitizeText` 改为**保留 UTF-8**(原 `asciiOnly` 把中文折成空格);MAIN 视图 msg/transcript、prompt 的 tool/hint、"Now" 行、历史浮层条目全部改 wqy14 + `drawUTF8`,并按 **UTF-8 字符边界**截断(原按字节截断会把中文切断成乱码)。新增 `clipUTF8` + UTF-8 感知的 `drawWrappedText`
  - ⬜ USAGE / SYSTEM:技术诊断为主,暂保留英文(后续按需)
...
| 2026-05-28 | (本次) | 阶段 3.3:CLOCK 视图中文化(wqy14 GB2312,星期/日期/卡标签/提示)。flash 29.5%,已烧录。菜单待做 |
| 2026-05-28 | (本次) | 阶段 3.3 完成:设置菜单(标题/标签/开关/确认页)中文化。已烧录 |
| 2026-05-28 | (本次) | 修复 MAIN 视图收到消息的中文乱码:sanitizeText 保留 UTF-8 + 消息渲染全改 wqy14/drawUTF8 + UTF-8 边界截断(clipUTF8)。已烧录 |
