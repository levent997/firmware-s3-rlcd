...
- 🔵 **3.3 中文字体** — 进行中。字体用 `u8g2_font_wqy14_t_gb2312`(文泉驿 14px,全 GB2312,含 ASCII,经 `drawUTF8` 渲染),flash 23.5%→29.5%(+~250KB,4MB 槽内无压力)
  - ✅ **CLOCK 视图**:星期(星期X)、日期(YYYY年M月D日)、元数据(第N周 / 全年第N天)、三张卡标签(温度/湿度/电量)、无时间提示全中文化。大字 HH:MM:SS 数字与秒 tick 保留
  - ⬜ **设置菜单**:LABELS / 开关值 / 确认标题待中文化(drawMenu + menu.cpp,确认页长说明拟保留英文)
  - ⬜ MAIN / USAGE / SYSTEM:技术诊断为主,暂保留英文
